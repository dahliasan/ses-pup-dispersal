...
for(i in 1:length(unique(sdive$ref))) {
  ...
  if(class(tmp)[1]!='try-error') {
    combFit <- try(fitDrate(tmp, weight='combWeight', prefilter=0.2), silent=T)
    augFit <- try(fitDrate(tmp, weight='augWeight', prefilter=0.2), silent=T)
...
png('augFit_all.png', width=20, height=16, units='cm',
    res=500, pointsize=10)
par(mfrow = c(7, 5), mar = c(2, 4, 2, 1))
for(i in 1:length(names(allFit))){
...
# Process Dives - testing bed ----------------------------------------

allFit <- list()
folder <- './Output/drift dive/processDive/allFit - trial/'
for(i in 1:length(unique(sdive$ref))) {
  print(i)
  tmp <- try(augmentWt(sdive %>% filter(ref==levels(sdive$ref)[i]), yLims=c(-0.1, 0.1) ), silent=T)
  if(class(tmp)[1]!='try-error') {
    ...
    if(class(Fit)!='try-error') {
      ...
      mtext(side=3, paste(levels(sdive$ref)[i], ' -', sep = ''))
      dev.off()
    }
  }
  allFit[[i]] <- list(augFit=augFit)
}

names(allFit) <- levels(sdive$ref)
purrr::map(allFit, function(x) x[[1]] %>% class) %>% unlist() -> results
sum(results == "try-error")

png('augFit_prefilter=0.25_all.png', width=20, height=16, units='cm',
    res=500, pointsize=10)
par(mfrow = c(7, 5), mar = c(2, 4, 2, 1))
for(i in 1:length(names(allFit))){
